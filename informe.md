...
Las lecturas reverse presentan un patrón similar al observado en las lecturas forward. La calidad inicial es alta y estable, mientras que las regiones terminales muestran una disminución progresiva de los scores de calidad (Figura 2). Este comportamiento puede introducir errores en análisis posteriores, como alineamientos o inferencias filogenéticas, si las lecturas no son previamente procesadas. 3)	trimming con fastp
...
La tabla 4 resume las métricas de calidad obtenidas para las lecturas reverse después del preprocesamiento. 
...
Las lecturas reverse procesadas presentan una mejora general en los valores de calidad respecto al dataset inicial. Se evidencia una reducción de las regiones con scores bajos y una mayor estabilidad en los valores Phred, indicando que el preprocesamiento eliminó exitosamente secuencias problemáticas y mejoró la calidad global del dataset (Figura 4).
...
La Figura 5 representa un árbol filogenético basado en secuencias 16S rRNA de distintas especies del género *Mycobacterium* y *Mycolicibacterium*. Cada nodo terminal (hoja) corresponde a una especie, y las ramas representan su relación evolutiva estimada a partir de la similitud genética.
